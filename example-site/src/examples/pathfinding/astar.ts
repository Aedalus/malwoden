...
export class AStarExample implements IExample {
  mount: HTMLElement;
  animRef: number;
  ...
  constructor() {
    ...
    this.astar = new Pathfinding.AStar({
    ...
    this.path = this.astar.compute(this.player, { x: 0, y: 0 });
  ...
  updatePath(newMouse: Vector2) {
    if (this.prevMouse.x === newMouse.x && this.prevMouse.y === newMouse.y)
      ...
      this.path = this.astar.compute(this.player, newMouse);
